...
export default function ViagensDoVeiculo() {
	const { id } = useParams();
	...
	async function listarViagens(id: string) {
		try {
			...
			const { data: resposta } = await viagemService.buscarViagensDoVeiculo(id, query);
	...
	return (
		<div className="w-full flex flex-col items-center pt-6 min-h-[calc(100vh-96px)] bg-gray-100">
			...
			<Link className="underline text-blue-600 cursor-pointer mb-6" to={`/veiculos/buscar/${valor}`}>
